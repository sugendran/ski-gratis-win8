...
(function(window, document, console) {
    ...
    function newWorld(stage, stageWidth, stageHeight) {
        console.log("stage is " + stageWidth + "x" + stageHeight);
        // "Constants"
        var TILE_EDGE = 64;
        ...
        var _speedUpInterval;
        var _crashTestTimeout;
        ...
        var bear = new createjs.BitmapAnimation(bearSpriteSheet);
        ...
        function speedUpThing() {
            if (!_isPaused)
                _offsetYSpeedUp = Math.min(8, _offsetXPerTick + (_offsetYPerTick * 0.5));
        }
        _speedUpInterval = setInterval(speedUpThing, 1000);
        ...
        function checkForCollision() {
            ...
            if(obj) {
                ...
                _offsetYSpeedUp = 0;
                _score -= 50;
                _crashTestTimeout = setTimeout(function () {
        ...
        function gameOver() {
            ...
            gameovertext.y = (stageHeight / 2) - 126;
            gameovertext.font = "bold 96px monospace";
            skierContainer.addChild(gameovertext);
            gameovertext.text = "GAME OVER";

            var restartText = new createjs.Text();
            restartText.textAlign = "center";
            restartText.x = stageWidth / 2;
            restartText.y = (stageHeight / 2) + 126;
            restartText.font = "bold 96px monospace";
            skierContainer.addChild(restartText);
            restartText.text = "RESTARTING IN 3";

            createjs.Tween.get(restartText)
                .wait(1000).set({ text: "RESTARTING IN 2" })
                .wait(1000).set({ text: "RESTARTING IN 1" })
                .wait(1000).call(function () {
                    skierContainer.removeChild(gameovertext);
                    skierContainer.removeChild(restartText);
                    _crashed = false;
                    _score = 0;
                    _offsetYSpeedUp = 0;
                    crash.alpha = 0;
        ...
        this.tick = function (elapsed) {
            ...
            if(_score > 10000 && !_bearUnleashed) {
...
//            fpsDisplay.text = "fps: " + ~~createjs.Ticker.getMeasuredFPS();
        ...
        this.resizeTo = function (w, h) {
            stageHeight = h;
            stageWidth = w;
            _maxCols = Math.floor(w / TILE_EDGE) + 16;
            _maxRows = Math.floor(h / TILE_EDGE) + 8;
            skier.x = (w * 0.5);
            skier.y = (h * 0.5);
            crash.x = (w * 0.5);
            crash.y = (h * 0.5);
    ...
    function loadSkiGratis(elementId) {
        ...
        var height = canvasElement.clientHeight * scale;
        var width = canvasElement.clientWidth * scale;
